...
public class CameraView extends CameraViewLayout {
    ...
    private OnFacingChangedListener facingListener;
    ...
    public CameraView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mCameraImpl = new Camera1(context, mEventDispatcher, mPreviewImpl);
    ...
    protected void onToggleFacing() {
        Log.i(TAG, "onToggleFacing: " + mDoubleTapToToggleFacing);
        if (mDoubleTapToToggleFacing) {
            facingListener.onFacingChanged();
            toggleFacing();
        }
    }
    ...
    public boolean getFlashSupport(@Facing final int facing) {
        return mCameraImpl.getFlashSupport(facing);
    }
...
*/
    ...
    public void captureVideo(Context context, CameraKitEventCallback<CameraKitVideo> callback, int maxDuration) {
        captureVideo(context, null, callback, maxDuration);
    }

    public void captureVideo(Context context, CameraKitEventCallback<CameraKitVideo> callback) {
        captureVideo(context, null, callback);
    }
    ...
    public void captureVideo(Context context, File videoFile, CameraKitEventCallback<CameraKitVideo> callback) {
        captureVideo(context, videoFile, callback, 0);
    }
    ...
    public void captureVideo(File videoFile, int maxDuration, final CameraKitEventCallback<CameraKitVideo> callback) {
        mCameraImpl.captureVideo(videoFile, maxDuration, new CameraImpl.VideoCapturedCallback() {
            ...
            public void onRecordStarted() {
                if (callback != null) callback.onRecordStarted();
            }

            @Override
            public void onRecordError() {
                if (callback != null) callback.onRecordError();
            }
        });
    }

    public void captureVideo(Context context, File videoFile, final CameraKitEventCallback<CameraKitVideo> callback, int maxDuration) {
        mCameraImpl.captureVideo(context, videoFile, maxDuration, new CameraImpl.VideoCapturedCallback() {
            @Override
            public void videoCaptured(File file) {
                CameraKitVideo video = new CameraKitVideo(file);
                if (callback != null) callback.callback(video);
    ...
    public void requestPermissions(boolean requestCamera, boolean requestAudio) {
    ...
    public void setFacingChangeListener(OnFacingChangedListener listener) {
        this.facingListener = listener;
    }

    public void setToggleFacing(boolean toggleFacing) {
        mDoubleTapToToggleFacing = toggleFacing;
    }

    public interface OnFacingChangedListener {
