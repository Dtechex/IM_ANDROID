package com.loopytime.external.story;
...
public class StoriesProgressView extends LinearLayout {
    ...
    public void skip() {
        if (isSkipStart || isReverseStart) {
            Log.i(TAG, "skip: isSkipStart || isReverseStart");
            return;
        }
        if (isComplete) {
            Log.i(TAG, "skip: isComplete");
            return;
        }
        if (current < 0) {
            Log.i(TAG, "skip: " + current);
            return;
        }
        PausableProgressBar p = progressBars.get(current);
        isSkipStart = true;
        Log.i(TAG, "skip: isSkipStart");
        p.setMax();
    }
    ...
    public void reverse() {
        if (isSkipStart || isReverseStart) {
            Log.i(TAG, "reverse: isSkipStart || isReverseStart");
            return;
        }
        if (isComplete) {
            Log.i(TAG, "reverse: isComplete");
            return;
        }
        if (current < 0) {
            Log.i(TAG, "reverse: " + current);
            return;
        }
        PausableProgressBar p = progressBars.get(current);
        Log.i(TAG, "reverse: isReverseStart ");
        isReverseStart = true;
        p.setMin();
    }
     ...
     * Start progress animation
     */
    public void startCurrentStories(int current) {
        Log.i(TAG, "startCurrentStories: " + this.current + ", " + current);
        progressBars.get(current).startProgress();
    }
    ...
    public void startMaxStories(int from) {
        for (int i = 0; i < from; i++) {
            progressBars.get(i).setMaxProgress(true);
        }
    }
     ...
     * Pause story
     */
    public void pause(int counter) {
        if (counter < 0) return;
        progressBars.get(counter).pauseProgress();
    }
