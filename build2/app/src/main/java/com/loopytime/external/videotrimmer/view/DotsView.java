...
public class DotsView extends View {
    private static final int DOTS_COUNT = 11;
    ...
    private void drawOuterDotsFrame(@NonNull Canvas canvas) {
    ...
    private void drawInnerDotsFrame(@NonNull Canvas canvas) {
        for (int i = 0; i < DOTS_COUNT; i++) {
    ...
    private void updateInnerDotsPosition() {
        if (currentProgress < 0.3f) {
            this.currentRadius2 = (float) UtilsAnim.mapValueFromRangeToRange(currentProgress, 0, 0.3f, 0.f, maxInnerDotsRadius);
        } else {
        ...
        if (currentProgress < 0.2) {
            ...
            this.currentDotSize2 = (float) UtilsAnim.mapValueFromRangeToRange(currentProgress, 0.2f, 0.5f, maxDotSize, 0.3 * maxDotSize);
        } else {
            this.currentDotSize2 = (float) UtilsAnim.mapValueFromRangeToRange(currentProgress, 0.5f, 1f, maxDotSize * 0.3f, 0);
        }
    ...
    private void updateOuterDotsPosition() {
        if (currentProgress < 0.3f) {
            this.currentRadius1 = (float) UtilsAnim.mapValueFromRangeToRange(currentProgress, 0.0f, 0.3f, 0, maxOuterDotsRadius * 0.8f);
        } else {
            this.currentRadius1 = (float) UtilsAnim.mapValueFromRangeToRange(currentProgress, 0.3f, 1f, 0.8f * maxOuterDotsRadius, maxOuterDotsRadius);
        }

        if (currentProgress < 0.7) {
            this.currentDotSize1 = maxDotSize;
        } else {
            this.currentDotSize1 = (float) UtilsAnim.mapValueFromRangeToRange(currentProgress, 0.7f, 1f, maxDotSize, 0);
        }
    }

    private void updateDotsPaints() {
        if (currentProgress < 0.5f) {
            float progress = (float) UtilsAnim.mapValueFromRangeToRange(currentProgress, 0f, 0.5f, 0, 1f);
            ...
            float progress = (float) UtilsAnim.mapValueFromRangeToRange(currentProgress, 0.5f, 1f, 0, 1f);
    ...
    private void updateDotsAlpha() {
        float progress = (float) UtilsAnim.clamp(currentProgress, 0.6f, 1f);
        int alpha = (int) UtilsAnim.mapValueFromRangeToRange(progress, 0.6f, 1f, 255, 0);
    ...
    public static final Property<DotsView, Float> DOTS_PROGRESS = new Property<DotsView, Float>(Float.class, "dotsProgress") {
        @Override
        public Float get(@NonNull DotsView object) {
            return object.getCurrentProgress();
        }

        @Override
        public void set(@NonNull DotsView object, Float value) {
