...
import static com.loopytime.im.MainActivity.loadPost;
...
public class PostFragment extends Fragment {
    ...
    public static ArrayList<FeedObject> listData = new ArrayList<>();
    Drawable dr, dr2;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        bundle.putString(FirebaseAnalytics.Param.ITEM_NAME, "Showing Post List ");
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        res = inflater.inflate(R.layout.fragment_post, container, false);
    ...
    void setListListener() {
        list.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                System.out.println("mandan "+stopLoadMore +" "+ loaded +" "+ loadMore);
                if (!stopLoadMore && loaded && loadMore
                        && totalItemCount <= (lastVisibleItem + visibleThreshold)) {
                    loadMore = false;
                    list.getAdapter().notifyItemChanged(listData.size() - 1);
    ...
    public void setData() {
        ...
        res.findViewById(R.id.no_item).setVisibility(View.GONE);
        ...
        loadMore = true;
        new ExecuteServices().execute(Constants.NODE_URL_GET + "get_feeds_hiddy/" + GetSet.getUserId(), new ExecuteServices.OnServiceExecute() {
            @Override
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            for (int i = 0; i < jAr.length(); i++) {
                                ...
                                listData.add(new FeedObject(job.getString("atext1"), job.getString("atext2"), job.getString("amedia"), job.getString("acreated_at"), job.getString("auname"), job.getString("aupic"), job.getString("auphone"), job.getString("aid"), job.getString("auid"), new FeedCommentObject(job.getString("comment"), job.getString("post_id"), job.getString("pid"), job.getString("created_at"), job.getString("uname"), job.getString("upic"), job.getString("uphone"), job.getString("id"), job.getString("uid")), job.isNull("count") ? "0" : job.getString("count"), job.isNull("ckount") ? "0" : job.getString("ckount"), job.getInt("afile_size"), media_type, 0));
                            }
                            getActivity().runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                            ...
                            getActivity().runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    Toast.makeText(getActivity(), "woops Something wrong happen"+e.getLocalizedMessage(), Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
                        ...
                        if (list.getAdapter().getItemCount() == 0) {
                            res.findViewById(R.id.no_item).setVisibility(View.VISIBLE);
                        }
                        if (list.getAdapter().getItemCount() < 5) {
                            stopLoadMore = true;
                        }
    ...
    public void setDataMore() {
        ...
        System.out.println("mandanz "+GetSet.getUserId());
        // Toast.makeText(getActivity(), "setDta", Toast.LENGTH_SHORT).show();
        new ExecuteServices().execute(Constants.NODE_URL_GET + "get_feeds_more_private_hiddy/" + listData.size() + "/" + GetSet.getUserId(), new ExecuteServices.OnServiceExecute() {
            @Override
            public void onServiceExecutedResponse(String response) {
                System.out.println("mandan "+response);
                if (getActivity() == null) return;
                getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ...
                        list.getAdapter().notifyDataSetChanged();
                        loadMore = true;
                        //list.getAdapter().notifyItemChanged(listData.size() - insertCount - 1);

                        if (insertCount > 0) {
                            //  list.getAdapter().notifyItemRangeInserted(listData.size() - insertCount, listData.size() - 1);
                        }
    ...
    class FeedsAdapter extends RecyclerView.Adapter<FeedsAdapter.ViewHolder> {
        ...
        void setUpFeed(FeedsAdapter.ViewHolder holder, int position) {
            ...
            if (!loadMore && position == listData.size() - 1) {
            ...
            if (stopLoadMore && position == listData.size() - 1) {
      ...
      /*      if (position!=0  && position % 8 == 0) {
            ...
            if (getActivity() == null) return;
            FeedObject feed = listData.get(position);
            try {
                isLocal = Utils.isInteger(feed.created_at);
            ...
            showAvatar(feed.upic, holder.avatarView);
            if (feed.feedComment == null || TextUtils.isEmpty(feed.feedComment.created_at) || feed.feedComment.created_at.equalsIgnoreCase("null")) {
            ...
            //holder.avatarView.bind(null, feed.uname, Integer.parseInt(feed.uid));
            ...
            if (!TextUtils.isEmpty(feed.text1) && !feed.text1.equalsIgnoreCase("null")) {
                ...
                holder.download.setVisibility(View.VISIBLE);
                if (Utils.feedMediaExist(feed.media)) {
                    holder.download.setImageDrawable(AppCompatResources.getDrawable(ApplicationClass.getInstance(), R.drawable.ic_share));
                }

               /* Glide.with(getActivity()).
                        load(Uri.parse("http://134.209.155.176:5040/pic_st/" + feed.media + ".png"
        ...
        void showAvatar(String url, ImageView avatarView) {
            Glide.with(getActivity()).applyDefaultRequestOptions(new RequestOptions().placeholder(R.drawable.temp)).load(url).
                    into(avatarView);
        }
        ...
        class ViewHolder extends RecyclerView.ViewHolder {
            ...
            public ViewHolder(View itemView) {
                ...
                //adView = itemView.findViewById(R.id.adViewItem);
              ...
              /*  container.post(new Runnable() {
                ...
                //((ImageView) itemView.findViewById(R.id.history)).setColorFilter(color.toActionBarColor(AndroidContext.getContext()));
                ((ImageView) itemView.findViewById(R.id.history)).setVisibility(View.VISIBLE);
                itemView.findViewById(R.id.history).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent intent = new Intent(getActivity(), WhatStatusFragmentFeedProfileActivity.class);
                        intent.putExtra("uid", listData.get(getAdapterPosition()).uid);
                        intent.putExtra("name", listData.get(getAdapterPosition()).uname);
                ...
                itemView.findViewById(R.id.progressBg).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        //FeedObject feed = listData.get(getAdapterPosition());
                ...
                itemView.findViewById(R.id.delete).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        long rid = Long.parseLong(listData.get(getAdapterPosition())._id);
                ...
                itemView.findViewById(R.id.download).setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (!TextUtils.isEmpty(feed.text1) && !feed.text1.equalsIgnoreCase("null")) {
                            ...
                            if (Utils.feedMediaExist(feed.media)) {
                                System.out.println("feeed media " + feed.media);

                                Utils.shareFeedMedia(Utils.getFeedUrl(feed.media), !feed.media.startsWith("vid_"), getActivity(), false);

                            } else {
                                /*if (!whatsList.contains(getAdapterPosition())) {
                                    whatsList.add(getAdapterPosition());
                                }*/
                                //FeedObject feed = listData.get(getAdapterPosition());
                                new DownloadFileFromURL().execute(feed.media, "0");
                            ...
                            /*if (feedMediaExist(feed.media)) {
                                ...
                                shareFeedMedia(feedMediaDrawableUrl(feed.media, false), !feed.media.startsWith("vid_"), getActivity(), false);
                ...
                whatsapp.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (!TextUtils.isEmpty(feed.text1) && !feed.text1.equalsIgnoreCase("null")) {
                            ...
                            if (Utils.feedMediaExist(feed.media)) {
                                System.out.println("feeed media " + feed.media);
                                Utils.shareFeedMedia(Utils.getFeedUrl(feed.media), !feed.media.startsWith("vid_"), getActivity(), true);
                            ...
                            /*if (!whatsList.contains(getAdapterPosition())) {
                                ...
                                //FeedObject feed = listData.get(getAdapterPosition());
                                new DownloadFileFromURL().execute(feed.media, "1");
                ...
                container.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                       ...
                       // WhatStatusFragment.loadData = false;
                        ...
                        intent1.putExtra("position",clickedItem);
                        intent1.putExtra("showComment", true);
...
//                avatarView.init(Screen.dp(38), 20);
 ...
 /*       ObjectAnimator starScaleYAnimator = ObjectAnimator.ofFloat(ivStar, ImageView.SCALE_Y, 0.2f, 1f);
    ...
    class DownloadFileFromURL extends AsyncTask<String, String, String> {
        ...
        protected void onPreExecute() {
            ...
            bundle.putString(FirebaseAnalytics.Param.ITEM_NAME, "Downloading post ");
        ...
        protected String doInBackground(String... f_url) {
            ...
            try {
                ...
                root = new File(externalPath + "/Indian-Messenger/feeds/");
                ...
                fromWhatsApp = f_url[1].equalsIgnoreCase("1");
        ...
        protected void onProgressUpdate(String... values) {
            ...
            pDialog2.setMessage("Downloading..." + values[0]);
        }
        ...
        protected void onPostExecute(String file_url) {
            ...
            if (file_url != null) {
                ...
                if (fromWhatsApp) {
                    Utils.shareFeedMedia(file_url, !new File(file_url).getName().startsWith("vid_"), getActivity(), true);
                    list.getAdapter().notifyDataSetChanged();
                }
            }
        }
    ...
    void sendReaction(String reaction, int pos) {
        if (getActivity() == null) return;
        RequestBody requestBody = new MultipartBody.Builder()

                .setType(MultipartBody.FORM)
                .addFormDataPart("reaction", reaction)
                .addFormDataPart("post_id", listData.get(pos)._id)
                .addFormDataPart("ct_id", listData.get(pos).uid)
                                         ...
                                         if (obj.getBoolean("success")) {
                                             ...
                                             if (obj.getInt("data") == 1) {
                                                 ...
                                                 getActivity().runOnUiThread(new Runnable() {
                                                     @Override
                                                     public void run() {
                                                         listData.get(pos).reaction_count = String.valueOf(Integer.parseInt(listData.get(pos).reaction_count) + 1);
                ...
                if (getActivity() != null)
                    if (clickedItem != -1) {
                        if (data.getBooleanExtra("deleted", false)) {
                            ...
                            listData.remove(clickedItem);
                            return;
                        }
                        if (data.getBooleanExtra("commentDeleted", false)) {
                            listData.set(clickedItem, (FeedObject) data.getSerializableExtra("obj"));
